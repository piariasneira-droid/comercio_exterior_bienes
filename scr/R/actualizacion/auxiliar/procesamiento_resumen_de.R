instala_carga_librerias <- function(librerias_necesarias) {
  ...
  for (lib in librerias_necesarias) {
    if (!requireNamespace(lib, quietly = TRUE)) {
      ...
      utils::install.packages(lib, dependencies = TRUE, repos = "https://cloud.r-project.org")
    }
  }
...
read_processed_data <- function(file_path, data_type) {
  ...
  if (data_type == "mes") {
    df[, Fecha := data.table::as.IDate(Fecha, format = "%Y-%m-%d")]
...
crear_tabla_de <- function(df, ano_filtro, mes_filtro) {
  get_value <- function(df_filtrado, region) {
    ...
    if (length(val) > 1) val <- val[1]  # safety: take first if duplicates
  ...
  resultado <- lapply(regiones, function(region) {
    ...
    tc_mes          <- ifelse(!is.na(exp_mes) && !is.na(imp_mes) && imp_mes != 0, 
                              round((exp_mes / imp_mes) * 100, 6), NA)
    ...
    tc_acum         <- ifelse(!is.na(exp_acum) && !is.na(imp_acum) && imp_acum != 0,
                              round((exp_acum / imp_acum) * 100, 6), NA)
    c(
      Región = ifelse(region == "Madrid, Comunidad de", "Madrid", "España"),
      Exportaciones = round(exp_mes, 6),
      `TVA (Exp)` = round(tva_exp_mes, 6),
      Importaciones = round(imp_mes, 6),
      `TVA (Imp)` = round(tva_imp_mes, 6),
      TC = tc_mes,
      `Exportaciones (YTM)` = round(exp_acum, 6),
      `TVA (Exp YTM)` = round(tva_exp_acum, 6),
      `Importaciones (YTM)` = round(imp_acum, 6),
      `TVA (Imp YTM)` = round(tva_imp_acum, 6),
      `TC (YTM)` = tc_acum
    )
  })
