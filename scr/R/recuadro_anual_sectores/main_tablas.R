...
source("./scr/R/recuadro_anual_sectores/auxiliar/funciones_gt.R")
...
arg_mad_sec <- list(
  ...
  titulo     = paste0("Comercio exterior de Madrid por sectores \u2014 ", params$anho),
  subtitulo  = paste0("Volumen (Mill.\u20ac), estructura porcentual, variaci\u00f3n anual y contribuci\u00f3n al crecimiento. Acumulado enero\u2013diciembre ", params$anho, "."),
  caption    = params$caption,
  fuente     = params$fuente_texto,
...
arg_esp_sec$titulo    <- paste0("Comercio exterior de Espa\u00f1a por sectores \u2014 ", params$anho)
arg_esp_sec$subtitulo <- paste0("Volumen (Mill.\u20ac), estructura porcentual, variaci\u00f3n anual y contribuci\u00f3n al crecimiento. Datos enero\u2013diciembre ", params$anho, ".")
...
arg_mad_pais$titulo    <- paste0("Comercio exterior de Madrid por pa\u00edses \u2014 ", params$anho)
arg_mad_pais$subtitulo <- paste0("Volumen (Mill.\u20ac), estructura porcentual, variaci\u00f3n anual y contribuci\u00f3n al crecimiento por pa\u00eds y zona geogr\u00e1fica. Datos enero\u2013diciembre ", params$anho, ".")
...
arg_esp_pais$titulo    <- paste0("Comercio exterior de Espa\u00f1a por pa\u00edses \u2014 ", params$anho)
arg_esp_pais$subtitulo <- paste0("Volumen (Mill.\u20ac), estructura porcentual, variaci\u00f3n anual y contribuci\u00f3n al crecimiento por pa\u00eds y zona geogr\u00e1fica. Datos enero\u2013diciembre ", params$anho, ".")

# Argumentos comunes evol ----
.arg_evol_base <- list(
  ano_base             = params$anho_idx,
  ano_final            = params$anho,
  anos_mostrar         = params$anho_idx:params$anho,
  cols_millones_factor = params$varfactor,
  dec_num              = params$dec_num,
  dec_pct              = params$dec_per,
  caption              = params$caption,
  fuente               = params$fuente_texto,
  tam_fuente           = params$gt_tam_fuente,
  ancho_cm             = params$gt_ancho_evol,
  dpi                  = params$dpi,
  col_pal              = params$gt_col_pal
)
...
tbl_sec_mad <- do.call(exportar_sectores_imagen, c(arg_mad_sec, list(
  ruta_salida = file.path(params$path_outt, sprintf("tabla_sectores_mad_%s.png", sufijo_m)),
  ancho_cm    = params$gt_ancho_tbl_mad,
  alto_cm     = params$gt_alto_tbl,
  dpi         = params$dpi
)))

### España ----
tbl_sec_esp <- do.call(exportar_sectores_imagen, c(arg_esp_sec, list(
  ruta_salida = file.path(params$path_outt, sprintf("tabla_sectores_esp_%s.png", sufijo_m)),
  ancho_cm    = params$gt_ancho_tbl,
  alto_cm     = params$gt_alto_tbl,
  dpi         = params$dpi
)))
...
tbl_pais_mad <- do.call(exportar_paises_imagen, c(arg_mad_pais, list(
  ruta_salida = file.path(params$path_outt, sprintf("tabla_paises_mad_%s.png",   sufijo_m)),
  ancho_cm    = params$gt_ancho_tbl,
  alto_cm     = params$gt_alto_tbl,
  dpi         = params$dpi
)))
...
tbl_pais_esp <- do.call(exportar_paises_imagen, c(arg_esp_pais, list(
  ruta_salida = file.path(params$path_outt, sprintf("tabla_paises_esp_%s.png",   sufijo_m)),
  ancho_cm    = params$gt_ancho_tbl,
  alto_cm     = params$gt_alto_tbl,
  dpi         = params$dpi
)))
...
df_evol_aux_sec <- df_evol_sec %>%
  filter(!orden %in% c(66L, params$filsec)) %>%
  mutate(
...
df_evol_aux_pais <- df_evol_pais %>%
  filter(!orden %in% c(72L, params$filpais)) %>%
  mutate(
...
tbl_evol_sec_mad_exp <- do.call(exportar_evol_imagen, c(.arg_evol_base, list(
  ...
  titulo      = paste0("Exportaciones de Madrid por sectores \u2014 Evoluci\u00f3n ", params$anho_idx, "\u2013", params$anho),
  subtitulo   = NULL,
  ruta_salida = file.path(params$path_outt, sprintf("evol_exp_mad_sec_%s.png", sufijo_evol))
)))

tbl_evol_sec_mad_imp <- do.call(exportar_evol_imagen, c(.arg_evol_base, list(
  tabla       = df_evol_aux_sec,
  flujo       = "imp", territorio = "mad", tipo = "sectores",
  titulo      = paste0("Importaciones de Madrid por sectores \u2014 Evoluci\u00f3n ", params$anho_idx, "\u2013", params$anho),
  subtitulo   = NULL,
  ruta_salida = file.path(params$path_outt, sprintf("evol_imp_mad_sec_%s.png", sufijo_evol))
)))
...
tbl_evol_sec_esp_exp <- do.call(exportar_evol_imagen, c(.arg_evol_base, list(
  ...
  titulo      = paste0("Exportaciones de Espa\u00f1a por sectores \u2014 Evoluci\u00f3n ", params$anho_idx, "\u2013", params$anho),
  subtitulo   = NULL,
  ruta_salida = file.path(params$path_outt, sprintf("evol_exp_esp_sec_%s.png", sufijo_evol))
)))

tbl_evol_sec_esp_imp <- do.call(exportar_evol_imagen, c(.arg_evol_base, list(
  tabla       = df_evol_aux_sec,
  flujo       = "imp", territorio = "esp", tipo = "sectores",
  titulo      = paste0("Importaciones de Espa\u00f1a por sectores \u2014 Evoluci\u00f3n ", params$anho_idx, "\u2013", params$anho),
  subtitulo   = NULL,
  ruta_salida = file.path(params$path_outt, sprintf("evol_imp_esp_sec_%s.png", sufijo_evol))
)))
...
tbl_evol_pais_mad_exp <- do.call(exportar_evol_imagen, c(.arg_evol_base, list(
  ...
  titulo      = paste0("Exportaciones de Madrid por pa\u00edses \u2014 Evoluci\u00f3n ", params$anho_idx, "\u2013", params$anho),
  subtitulo   = NULL,
  ruta_salida = file.path(params$path_outt, sprintf("evol_exp_mad_pais_%s.png", sufijo_evol))
)))

tbl_evol_pais_mad_imp <- do.call(exportar_evol_imagen, c(.arg_evol_base, list(
  tabla       = df_evol_aux_pais,
  flujo       = "imp", territorio = "mad", tipo = "paises",
  titulo      = paste0("Importaciones de Madrid por pa\u00edses \u2014 Evoluci\u00f3n ", params$anho_idx, "\u2013", params$anho),
  subtitulo   = NULL,
  ruta_salida = file.path(params$path_outt, sprintf("evol_imp_mad_pais_%s.png", sufijo_evol))
)))
...
tbl_evol_pais_esp_exp <- do.call(exportar_evol_imagen, c(.arg_evol_base, list(
  ...
  titulo      = paste0("Exportaciones de Espa\u00f1a por pa\u00edses \u2014 Evoluci\u00f3n ", params$anho_idx, "\u2013", params$anho),
  subtitulo   = NULL,
  ruta_salida = file.path(params$path_outt, sprintf("evol_exp_esp_pais_%s.png", sufijo_evol))
)))

tbl_evol_pais_esp_imp <- do.call(exportar_evol_imagen, c(.arg_evol_base, list(
  tabla       = df_evol_aux_pais,
  flujo       = "imp", territorio = "esp", tipo = "paises",
  titulo      = paste0("Importaciones de Espa\u00f1a por pa\u00edses \u2014 Evoluci\u00f3n ", params$anho_idx, "\u2013", params$anho),
  subtitulo   = NULL,
  ruta_salida = file.path(params$path_outt, sprintf("evol_imp_esp_pais_%s.png", sufijo_evol))
)))
...
tbl_evol_pct_sec_exp <- do.call(exportar_evol_pct_imagen, c(.arg_evol_pct_base, list(
  ...
  subtitulo   = NULL,
  ruta_salida = file.path(params$path_outt, sprintf("evol_pct_exp_sec_%s.png", sufijo_evol))
)))
...
tbl_evol_pct_sec_imp <- do.call(exportar_evol_pct_imagen, c(.arg_evol_pct_base, list(
  ...
  subtitulo   = NULL,
  ruta_salida = file.path(params$path_outt, sprintf("evol_pct_imp_sec_%s.png", sufijo_evol))
)))
...
tbl_evol_pct_pais_exp <- do.call(exportar_evol_pct_imagen, c(.arg_evol_pct_base, list(
  ...
  subtitulo   = NULL,
  ruta_salida = file.path(params$path_outt, sprintf("evol_pct_exp_pais_%s.png", sufijo_evol))
)))
...
tbl_evol_pct_pais_imp <- do.call(exportar_evol_pct_imagen, c(.arg_evol_pct_base, list(
  ...
  subtitulo   = NULL,
  ruta_salida = file.path(params$path_outt, sprintf("evol_pct_imp_pais_%s.png", sufijo_evol))
)))
...
do.call(exportar_sectores_excel, c(
  ...
  list(ruta_salida = file.path(params$path_outx, sprintf("tablagt_sec_mad_%s.xlsx",  sufijo_m)))
))

do.call(exportar_sectores_excel, c(
  .filtrar_args(arg_esp_sec,  .solo_imagen),
  list(ruta_salida = file.path(params$path_outx, sprintf("tablagt_sec_esp_%s.xlsx",  sufijo_m)))
))

do.call(exportar_paises_excel, c(
  .filtrar_args(arg_mad_pais, .solo_imagen),
  list(ruta_salida = file.path(params$path_outx, sprintf("tablagt_pais_mad_%s.xlsx", sufijo_m)))
))

do.call(exportar_paises_excel, c(
  .filtrar_args(arg_esp_pais, .solo_imagen),
  list(ruta_salida = file.path(params$path_outx, sprintf("tablagt_pais_esp_%s.xlsx", sufijo_m)))
))
...
exportar_evol_excel(
  ...
  flujo       = "exp", territorio = "mad", tipo = "sectores",
  titulo      = paste0("Exportaciones de Madrid por sectores \u2014 Evoluci\u00f3n ", params$anho_idx, "\u2013", params$anho),
  ano_base    = params$anho_idx, ano_final = params$anho, anos_mostrar = params$anho_idx:params$anho,
  cols_millones_factor = params$varfactor,
  dec_num = params$dec_num, dec_pct = params$dec_per,
  caption = params$caption, tam_fuente = params$gt_tam_fuente, col_pal = params$gt_col_pal,
  ruta_salida = file.path(params$path_outx, sprintf("tablagt_evol_exp_mad_sec_%s.xlsx",  sufijo_evol))
)
...
exportar_evol_excel(
  ...
  dec_num = params$dec_num, dec_pct = params$dec_per,
  caption = params$caption, tam_fuente = params$gt_tam_fuente, col_pal = params$gt_col_pal,
  ruta_salida = file.path(params$path_outx, sprintf("tablagt_evol_imp_mad_sec_%s.xlsx",  sufijo_evol))
)
...
exportar_evol_excel(
  ...
  dec_num = params$dec_num, dec_pct = params$dec_per,
  caption = params$caption, tam_fuente = params$gt_tam_fuente, col_pal = params$gt_col_pal,
  ruta_salida = file.path(params$path_outx, sprintf("tablagt_evol_exp_esp_sec_%s.xlsx",  sufijo_evol))
)
...
exportar_evol_excel(
  ...
  dec_num = params$dec_num, dec_pct = params$dec_per,
  caption = params$caption, tam_fuente = params$gt_tam_fuente, col_pal = params$gt_col_pal,
  ruta_salida = file.path(params$path_outx, sprintf("tablagt_evol_imp_esp_sec_%s.xlsx",  sufijo_evol))
)
...
exportar_evol_excel(
  ...
  dec_num = params$dec_num, dec_pct = params$dec_per,
  caption = params$caption, tam_fuente = params$gt_tam_fuente, col_pal = params$gt_col_pal,
  ruta_salida = file.path(params$path_outx, sprintf("tablagt_evol_exp_mad_pais_%s.xlsx", sufijo_evol))
)
...
exportar_evol_excel(
  ...
  dec_num = params$dec_num, dec_pct = params$dec_per,
  caption = params$caption, tam_fuente = params$gt_tam_fuente, col_pal = params$gt_col_pal,
  ruta_salida = file.path(params$path_outx, sprintf("tablagt_evol_imp_mad_pais_%s.xlsx", sufijo_evol))
)
...
exportar_evol_excel(
  ...
  dec_num = params$dec_num, dec_pct = params$dec_per,
  caption = params$caption, tam_fuente = params$gt_tam_fuente, col_pal = params$gt_col_pal,
  ruta_salida = file.path(params$path_outx, sprintf("tablagt_evol_exp_esp_pais_%s.xlsx", sufijo_evol))
)
...
exportar_evol_excel(
  ...
  dec_num = params$dec_num, dec_pct = params$dec_per,
  caption = params$caption, tam_fuente = params$gt_tam_fuente, col_pal = params$gt_col_pal,
  ruta_salida = file.path(params$path_outx, sprintf("tablagt_evol_imp_esp_pais_%s.xlsx", sufijo_evol))
)
