...
# Aseguramos el sufijo del mes tal y como se genera en tus procesos
sufijo_mes <- sprintf("%04d_%02d", paramets$anho, paramets$mes) 
# Nota: Si tu script usa otro formato (ej. "03"), asegúrate de que cuadre aquí.
...
inyectar_imagen_objeto <- function(tmp_dir, marcador, ruta_img, id_rel, ancho_cm, alinear_h = "right", alinear_v = "top", wrap_mode = "square") {
  ...
  xml_doc      <- xml2::read_xml(ruta_doc_xml) # <--- Forzado xml2
  ns_w         <- c(w = "http://schemas.openxmlformats.org/wordprocessingml/2006/main")
  todos_los_p  <- xml2::xml_find_all(xml_doc, "//w:p", ns_w) # <--- Forzado xml2
  
  remplazado <- FALSE
  for (p in todos_los_p) {
    if (grepl(marcador, xml2::xml_text(p), fixed = TRUE)) { # <--- Forzado xml2
      xml2::xml_add_sibling(p, xml2::read_xml(xml_anchor), .where = "before") # <--- Forzado xml2
      xml2::xml_remove(p) # <--- Forzado xml2
      remplazado <- TRUE
      break
    }
  }
  ...
  if (remplazado) {
    xml2::write_xml(xml_doc, ruta_doc_xml) # <--- Forzado xml2
    return(TRUE)
  }
}
...
# Mapeo estructurado de todo el layout solicitado para las 8 páginas
plan_mapeo <- list(
  # Pagina 1
  list(m = "MARCADOR_P1_PLOT1",   f = "plot1_mad_evo_mes.png",                 id = "rIdP1Plot",  w = 8.5,  h = "right",  v = "top",    wr = "square"),
  list(m = "MARCADOR_P1_CCAA",    f = "table_p1_t1.png",                       id = "rIdP1Ccaa",  w = 18.0, h = "center",   v = "bottom", wr = "none"),
  
  # Pagina 2
  list(m = "MARCADOR_P2_PLOT2",   f = "plot2_mad_mm12_anos.png",               id = "rIdP2Plot",  w = 18.0, h = "left",   v = "bottom", wr = "none"),
  
  # Pagina 3
  list(m = "MARCADOR_P3_CONTRIB", f = paste0("contrib_exp_mad_sec_", sufijo_mes, ".png"), id = "rIdP3Cont", w = 10,  h = "right",  v = "top",    wr = "square"),
  list(m = "MARCADOR_P3_TREEMAP", f = paste0("treemap_exp_mad_sec_", sufijo_mes, ".png"), id = "rIdP3Tree", w = 18.0, h = "left",   v = "bottom", wr = "none"),
  
  # Pagina 4
  list(m = "MARCADOR_P4_CONTRIB", f = paste0("contrib_imp_mad_sec_", sufijo_mes, ".png"), id = "rIdP4Cont", w = 10,  h = "right",  v = "top",    wr = "square"),
  list(m = "MARCADOR_P4_TREEMAP", f = paste0("treemap_imp_mad_sec_", sufijo_mes, ".png"), id = "rIdP4Tree", w = 18.0, h = "left",   v = "bottom", wr = "none"),
  # Pagina 5
  list(m = "MARCADOR_P5_CONTRIB", f = paste0("contrib_exp_mad_pais_", sufijo_mes, ".png"),id = "rIdP5Cont", w = 10,  h = "right",  v = "top",    wr = "square"),
  list(m = "MARCADOR_P5_TREEMAP", f = paste0("treemap_exp_mad_pais_", sufijo_mes, ".png"),id = "rIdP5Tree", w = 18.0, h = "left",   v = "bottom", wr = "none"),
  # Pagina 6
  list(m = "MARCADOR_P6_CONTRIB", f = paste0("contrib_imp_mad_pais_", sufijo_mes, ".png"),id = "rIdP6Cont", w = 10,  h = "right",  v = "top",    wr = "square"),
  list(m = "MARCADOR_P6_TREEMAP", f = paste0("treemap_imp_mad_pais_", sufijo_mes, ".png"),id = "rIdP6Tree", w = 18.0, h = "left",   v = "bottom", wr = "none"),
  # Paginas 7 y 8 (Spark Tables Completas)
  list(m = "MARCADOR_P7_SPARK",   f = paste0("tabla_sec_spark_mad_", sufijo_mes, ".png"), id = "rIdP7Sprk", w = 18.0, h = "center", v = "center", wr = "none"),
  list(m = "MARCADOR_P8_SPARK",   f = paste0("tabla_pais_spark_mad_", sufijo_mes, ".png"),id = "rIdP8Sprk", w = 18.0, h = "center", v = "center", wr = "none")
)
...
# EN VEZ DE SYSTEM: Usamos el paquete zip de R de forma portátil
...
cat("\n================================================================\n")
cat("¡Proceso completado con éxito estructural multicanal!\n")
cat("Documento final guardado en:", ruta_final_completa, "\n")
cat("================================================================\n")