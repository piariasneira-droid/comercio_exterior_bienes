...
for (pais in for_paises) {
  ...
  for (flujo in for_flujo) {
    ...
    ##### Recolección TARICs relevantes
    cat("    Recopilando TARICs únicos para flujo:", flujo, "...\n")
    fortarics <- bit64::integer64(0)
    ...
    for (nivel in for_niveles) {
      for (anio in for_aniotop) {
        ...
        fortarics <- union(fortarics, bit64::as.integer64(top_tarics))
      }
    }
    
    fortarics <- fortarics[fortarics != bit64::as.integer64(0L)]
    cat("    TARICs únicos encontrados:", length(fortarics), "\n")
    
    ##### Evoluciones temporales
    for (i in seq_along(fortarics)) {
      fil_taric <- fortarics[i] # Preserves integer64 class
      
      cat("    Procesando TARIC:", as.character(fil_taric), "| Flujo:", flujo, "\n")
      
      ###### Anual
      df_anual <- df_periodo_exposicion_asimetria(
        df_mad         = dfmad,
        df_esp         = dfesp,
        filtro_flujo   = flujo,
        ...
        top_paises     = n_paises,
        pais_analisis  = pais,
        filtro_ano_top = fil_aniotop,
        df_pais        = df_paises,
        df_taric       = df_tarics
      )
      
      if (nrow(df_anual) == 0) {
        cat("      SKIP anual (no data)\n")
        next
      }
      
      ruta_excel_anual <- file.path(
        outdir, "anuales/exceles",
        paste0("evol_anual",
               "_taric_", as.character(fil_taric),
               "_flujo_", flujo, ".xlsx")
      )
      openxlsx::write.xlsx(df_anual, ruta_excel_anual)
      
      plot_anual <- crear_grafico_lineas_dependencia_periodo(
        df       = df_anual,
        y_var    = "Grado dependencia",
        ctaric   = fil_taric,
        df_taric = df_tarics
      )
      htmltools::save_html(plot_anual, file.path(
        outdir, "anuales/htmls",
        paste0("grafico_dependencia_anual",
               "_taric_", as.character(fil_taric),
               "_flujo_", flujo, ".html")
      ))
      
      ###### Mensual
      df_mensual <- df_evolucion_exposicion_asimetria(
        df_mad         = dfmad,
        df_esp         = dfesp,
        filtro_flujo   = flujo,
        filtro_taric   = fil_taric,
      ...
      if (nrow(df_mensual) == 0) {
        cat("      SKIP mensual (no data)\n")
        next
      }
      
      ruta_excel_mensual <- file.path(
        outdir, "mensuales/exceles",
        paste0("evol_mensual",
               "_taric_", as.character(fil_taric),
               "_flujo_", flujo, ".xlsx")
      )
      openxlsx::write.xlsx(df_mensual, ruta_excel_mensual)
      
      plot_mensual <- crear_grafico_lineas_dependencia_evolucion(
        df       = df_mensual,
        y_var    = "Grado dependencia",
        ctaric   = fil_taric,
        df_taric = df_tarics
      )
      htmltools::save_html(plot_mensual, file.path(
        outdir, "mensuales/htmls",
        paste0("grafico_dependencia_mensual",
               "_taric_", as.character(fil_taric),
               "_flujo_", flujo, ".html")
      ))
      
      cat("      TARIC:", as.character(fil_taric), "| Flujo:", flujo, "- OK\n")
    } # closes: for (i in seq_along(fortarics))
