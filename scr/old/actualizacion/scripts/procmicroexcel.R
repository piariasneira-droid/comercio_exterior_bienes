...
obtener_df_ccaas <- function(path_ccaa, mapeo_regiones) {
  ...
  df[, flujo := data.table::fcase(
    columna == "EXPORT", "EXPORT",
    columna == "IMPORT", "IMPORT"
  )]
  
  # Convertir subfila (meses) a Fecha
  meses_esp <- c(
  ...
  df[, c("mes_num", "anio") := data.table::tstrsplit(subfila, " de ", fixed = TRUE)]
  df[, mes_num := meses_esp[mes_num]]
  df[, Fecha := as.POSIXct(paste(anio, mes_num, "01", sep = "-"), format = "%Y-%m-%d", tz = "UTC")]
  
  # Join con mapeo de regiones
  df <- mapeo_regiones[df, on = c("Cod" = "fila")]
  
  # Crear columnas ccaa y Condn desde el mapeo
  df_ccaa <- df[, .(
    ccaa = Región,
    Condn = Condn,
    flujo,
...
guardar_datos_brutos <- function(dt_ccaas, dt_tarics, dt_paises, dir_salida) {
  ...
  if (file.exists(file_salida)) {
    tryCatch({
      # Intentar eliminar el archivo (solo funciona si no está abierto)
      file.remove(file_salida)
    }, error = function(e) {
      # Si falla, intentar cerrarlo con COM (solo Windows)
      if (.Platform$OS.type == "windows") {
        tryCatch({
          excel_app <- RDCOMClient::COMCreate("Excel.Application")
          excel_app[["DisplayAlerts"]] <- FALSE
          
          # Buscar el workbook abierto
          for (i in 1:excel_app[["Workbooks"]][["Count"]]) {
            wb_open <- excel_app[["Workbooks"]][[i]]
            if (normalizePath(wb_open[["FullName"]], winslash = "/") == normalizePath(file_salida, winslash = "/")) {
          ...
          rm(excel_app)
          gc()
          Sys.sleep(1)  # Esperar un segundo
        }, error = function(e2) {
          warning("No se pudo cerrar el archivo automáticamente. Por favor, ciérralo manualmente.")
          stop("El archivo está abierto en Excel. Ciérralo y vuelve a ejecutar.")
        })
      } else {
        stop("El archivo está abierto. Por favor, ciérralo y vuelve a ejecutar.")
      }
    })
  }
  ...
  # Agregar hojas
  openxlsx::addWorksheet(wb, "df_ccaa")
  openxlsx::addWorksheet(wb, "df_taric")
  openxlsx::addWorksheet(wb, "df_paises")
  
  # Escribir los datos sin modificar valores
  openxlsx::writeData(wb, "df_ccaa", dt_ccaas)
  openxlsx::writeData(wb, "df_taric", dt_tarics)
  openxlsx::writeData(wb, "df_paises", dt_paises)
  
  # Crear estilo de formato numérico con separador de miles y 2 decimales
  style_valor <- openxlsx::createStyle(numFmt = "#,##0.00")
  
  # Aplicar estilo a cada hoja
  for (df_info in list(
    list(df = dt_ccaas, hoja = "df_ccaa"),
    list(df = dt_tarics, hoja = "df_taric"),
    list(df = dt_paises, hoja = "df_paises")
  )) {
    col_valor <- which(names(df_info$df) == "valor")
  ...
  # Guardar workbook
  openxlsx::saveWorkbook(wb, file_salida, overwrite = TRUE)
  
  message("Archivo guardado en: ", file_salida)
}

actualizar_exceles <- function(excel_path,
                               dt_ccaas,
                               dt_tarics,
                               dt_paises,
                               ccaa_exp,
  ...
  # Variable para saber si el archivo existe
  archivo_existe <- file.exists(excel_path)
  
  # Si el archivo existe, intentar cerrarlo si está abierto (solo Windows)
  if (archivo_existe && .Platform$OS.type == "windows") {
    tryCatch({
      excel_app <- RDCOMClient::COMCreate("Excel.Application")
      excel_app[["DisplayAlerts"]] <- FALSE
      ...
      if (excel_app[["Workbooks"]][["Count"]] > 0) {
        for (i in 1:excel_app[["Workbooks"]][["Count"]]) {
          wb_open <- excel_app[["Workbooks"]][[i]]
          if (normalizePath(wb_open[["FullName"]], winslash = "/") == 
              normalizePath(excel_path, winslash = "/")) {
            wb_open$Close(SaveChanges = FALSE)
            message("Archivo cerrado automáticamente")
            break
          }
        }
      }
      ...
      message("No se pudo cerrar el archivo automáticamente")
    })
  }
  
  # Convertir mapeos a data.table
  dt_ccaas <- data.table::as.data.table(dt_ccaas)
  dt_tarics <- data.table::as.data.table(dt_tarics)
  dt_paises <- data.table::as.data.table(dt_paises)
  
  # Preparar mapeos base con todas las categorías
  mapeo_ccaa_base <- dt_ccaas[, .(ccaa = Región, Condn)]
  mapeo_taric_base <- dt_tarics[, .(taric)]
  # Para países: incluir todos los nompais, incluso si pais es NA
  mapeo_pais_base <- unique(dt_paises[, .(pais = nompais)])
  
  # Función auxiliar para completar datos con mapeo
  completar_con_mapeo <- function(df_data, mapeo_base, join_cols) {
    df_data <- data.table::as.data.table(df_data)
    
    # Si df_data está vacío, devolver solo el mapeo con 0s
    if (nrow(df_data) == 0) {
      return(mapeo_base)
    }
    
    # Identificar qué filas del mapeo están en los datos
    df_completo <- data.table::copy(mapeo_base)
    
    # Merge a la izquierda: mapeo LEFT JOIN datos
    df_completo <- merge(df_completo, df_data, by = join_cols, all.x = TRUE)
    
    # Obtener columnas de fechas (todas menos las de join)
    cols_fechas <- setdiff(names(df_completo), join_cols)
    
    # Reemplazar NAs por 0 en columnas de fechas
    for (col in cols_fechas) {
      if (is.numeric(df_completo[[col]])) {
        data.table::set(df_completo, which(is.na(df_completo[[col]])), col, 0)
      }
    }
  ...
  # Completar cada dataframe con su mapeo correspondiente
  ccaa_exp_completo <- completar_con_mapeo(ccaa_exp, mapeo_ccaa_base, c("ccaa", "Condn"))
  ccaa_imp_completo <- completar_con_mapeo(ccaa_imp, mapeo_ccaa_base, c("ccaa", "Condn"))
  taric_exp_completo <- completar_con_mapeo(taric_exp, mapeo_taric_base, "taric")
  taric_imp_completo <- completar_con_mapeo(taric_imp, mapeo_taric_base, "taric")
  paises_exp_completo <- completar_con_mapeo(paises_exp, mapeo_pais_base, "pais")
  paises_imp_completo <- completar_con_mapeo(paises_imp, mapeo_pais_base, "pais")
  
  # Ordenar ccaas por Condn
  data.table::setorder(ccaa_exp_completo, Condn)
  data.table::setorder(ccaa_imp_completo, Condn)
  
  # Definir mapeo de dataframes a hojas
  hojas_datos <- list(
    "ccaaexp" = ccaa_exp_completo,
    "ccaaimp" = ccaa_imp_completo,
    "taricexp" = taric_exp_completo,
    "taricimp" = taric_imp_completo,
    "paisesexp" = paises_exp_completo,
    "paisesimp" = paises_imp_completo
  )
  
  # Crear o cargar el libro de trabajo
  if (archivo_existe) {
    wb <- openxlsx::loadWorkbook(excel_path)
    message("Cargando archivo existente")
  } else {
    wb <- openxlsx::createWorkbook()
    for (hoja_nombre in names(hojas_datos)) {
      openxlsx::addWorksheet(wb, sheetName = hoja_nombre)
    }
    message("Creando nuevo archivo")
  }
  
  # Crear estilos
  estilo_numerico <- openxlsx::createStyle(numFmt = "#,##0.00")
  estilo_encabezado <- openxlsx::createStyle(textDecoration = "bold")
  
  # Procesar cada hoja
  for (hoja_nombre in names(hojas_datos)) {
    df <- hojas_datos[[hoja_nombre]]
    
    # Crear hoja si no existe
    if (!hoja_nombre %in% names(wb)) {
      openxlsx::addWorksheet(wb, sheetName = hoja_nombre)
    }
    
    # Borrar contenido existente si el archivo ya existía
    if (archivo_existe) {
      openxlsx::deleteData(wb, sheet = hoja_nombre, 
                           cols = 1:200, rows = 1:10000, 
                           gridExpand = TRUE)
    }
    
    # Escribir datos
    openxlsx::writeData(wb, sheet = hoja_nombre, x = df, 
                        startRow = 1, startCol = 1)
    
    # Aplicar negrita a encabezados
    if (ncol(df) > 0) {
      openxlsx::addStyle(wb, sheet = hoja_nombre, 
    ...
    openxlsx::freezePane(wb, sheet = hoja_nombre, firstRow = TRUE)
    
    # Formatear columnas de fecha
    cols_fecha <- which(grepl("^\\d{4}-\\d{2}-\\d{2}$", names(df)))
    if (length(cols_fecha) > 0 && nrow(df) > 0) {
  ...
  message(paste("Excel guardado:", excel_path))
}
